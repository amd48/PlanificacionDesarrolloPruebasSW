...
{
    public boolean compruebaPermisos(String login, String pasword, Usuario tipoUsu)
    {
    ...
    public void realizaReserva(String login, String password, String socio, String [] isbns) throws ReservaException
    {
        ...
        {
            //FactoriaBOs fd = new FactoriaBOs(); CODIGO ORIGINAL
            FactoriaBOs fd = Factoria.create();   //CODIGO REFACTORIZADO
            IOperacionBO io = fd.getOperacion();
            ...
            try
            {
                for (String isbn: isbns)
                {
                    ...
                    catch (IsbnInvalido iie)
                    {
                        errores.add("ISBN invalido"+":"+isbn);
                    }
                }
            }
            catch (SocioInvalido sie)
            {
